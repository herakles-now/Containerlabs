...
containers → gateway sysctls → interface MTU → NAT tables → data path for all
four scenarios, including a TCP probe through the port-forward DNAT, and stops
at the lowest failing layer) or the matching `test-*`/`verify`, then `heal`.
