...
# Standby A: a read-only hot standby that streams WAL from the primary (pg1).
# Takes a base backup once, then starts in standby mode and follows the primary.
...
# 0. SSH so the backup host can read this standby's data files (backup-standby).
/usr/sbin/sshd
...
# 5. Start the standby (read-only, hot standby — it follows pg1 automatically)
